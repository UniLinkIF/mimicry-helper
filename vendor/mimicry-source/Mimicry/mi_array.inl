...
{
    array_internal::SZeroHelper< T, typename mSTypeTagHelper< T >::msTag >::Zero( a_Dest );
}
...
{
    return this->m_pElements[ this->m_uIndex ];
}
...
{
    return this->m_pElements[ this->m_uIndex + a_uIndex ];
}
...
{
    return ( this->m_uIndex == a_itOther.m_uIndex ) && ( this->m_pElements == a_itOther.m_pElements );
}
...
MIBool mTArray< T >::CIterator::operator < ( CIterator const & a_itOther ) const
{
    return ( this->m_pElements + this->m_uIndex ) < ( a_itOther.m_pElements + a_itOther.m_uIndex );
}
